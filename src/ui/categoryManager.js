...
function buildCategoryManagerHTML() {
  ...
  return '' +
    ...
    '<div class="category-count-badge" role="status" aria-live="polite">' + Object.keys(categories).length + '</div>' +
    '</div>' +
...
function buildCategoriesGrid(categories) {
  ...
  sortedCategories.forEach(([categoryName, categoryData], index) => {
    ...
    html += '<div class="category-card" data-category="' + categoryName + '" data-order="' + order + '" draggable="true">' +
      ...
      '<div class="category-color-display" style="background-color: ' + color + '"></div>' +
      '<div class="drag-handle">⋮⋮</div>' +
      '</div>' +
      '<div class="category-info">' +
      '<h5 class="category-name-display">' + categoryName + '</h5>' +
      '<span class="category-order">Order: ' + order + '</span>' +
      '</div>' +
      '<div class="category-actions">' +
      '<button class="btn-icon-small btn-edit" title="Edit category" data-category="' + categoryName + '">' +
      '✏️' +
      '</button>' +
      '<button class="btn-icon-small btn-delete" title="Delete category" data-category="' + categoryName + '">' +
      '🗑️' +
      '</button>' +
      '</div>' +
      '</div>' +
      ...
      (subCount > 0 ?
        '<button class="btn-subcategories" data-category="' + categoryName + '">' +
        '<span class="subcategory-icon">📁</span>' +
        '<span>Manage ' + subCount + ' subcategories</span>' +
        '<span class="expand-icon">▼</span>' +
        '</button>' :
        '<button class="btn-add-subcategory" data-category="' + categoryName + '">' +
      ...
      '<h6>Edit Category</h6>' +
      '<button class="btn-close-edit" data-category="' + categoryName + '">✕</button>' +
      '</div>' +
      '<div class="edit-form-body">' +
      '<div class="input-group-inline">' +
      '<div class="input-wrapper-small">' +
      '<label>Name</label>' +
      '<input type="text" class="edit-name-input" value="' + categoryName + '" data-category="' + categoryName + '">' +
      '</div>' +
      '<div class="input-wrapper-small">' +
      '<label>Color</label>' +
      '<input type="color" class="edit-color-input" value="' + color + '" data-category="' + categoryName + '">' +
      '</div>' +
      '<div class="input-wrapper-small">' +
      '<label>Order</label>' +
      '<input type="number" class="edit-order-input" value="' + order + '" data-category="' + categoryName + '" min="0" max="999">' +
      '</div>' +
      '</div>' +
      '<div class="edit-form-actions">' +
      '<button class="btn btn-success btn-sm btn-save-edit" data-category="' + categoryName + '">' +
      '<span class="btn-icon">💾</span> Save' +
      '</button>' +
      '<button class="btn btn-secondary btn-sm btn-cancel-edit" data-category="' + categoryName + '">' +
      'Cancel' +
      '</button>' +
      '</div>' +
      '</div>' +
      ...
      '<div class="subcategories-section" style="display: none;" data-category="' + categoryName + '">' +
...
function buildSubcategoriesSection(categoryName, subcategories) {
  let html = '' +
    ...
    '<h5><span class="subcategory-icon">📁</span> Subcategories for ' + categoryName + '</h5>' +
    '<button class="btn-close-subcategories" data-category="' + categoryName + '" title="Close subcategories">✕</button>' +
    '</div>' +
    ...
    '</div>' +
    '<div class="input-wrapper-small">' +
    '<button class="btn btn-success btn-sm add-subcategory-btn" data-parent="' + categoryName + '">' +
  ...
  if (Object.keys(subcategories).length > 0) {
    ...
    Object.entries(subcategories).forEach(function ([subName, subColor]) {
      html += '' +
        '<div class="subcategory-card" data-parent="' + categoryName + '" data-subcategory="' + subName + '">' +
        '<div class="subcategory-header">' +
        '<div class="subcategory-color" style="background-color: ' + subColor + '"></div>' +
        '<span class="subcategory-name">' + subName + '</span>' +
        '<div class="subcategory-actions">' +
        '<button class="btn-icon-mini btn-edit-subcategory" ' +
        'title="Edit subcategory"' +
        ' data-parent="' + categoryName + '" ' +
        ' data-subcategory="' + subName + '">✏️</button>' +
        ...
        ' data-parent="' + categoryName + '" ' +
        ' data-subcategory="' + subName + '">💾</button>' +
...
function attachCategoryManagerEventListeners(container, modal) {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      const visibleCount = Array.from(categoryCards).filter(card =>
      ...
      if (nameInput?.value && colorInput?.value) {
        ...
        if (newName) {
          ...
          import('./uiManager.js').then(module => {
            module.showToast('Category "' + newName + '" updated successfully', "success");
          });
        }
      }
    });
  ...
  container.querySelectorAll('.btn-subcategories').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const categoryCard = e.target.closest('.category-card');
      const subcategoriesSection = categoryCard.querySelector('.subcategories-section');
  ...
  container.querySelectorAll('.category-card').forEach(card => {
    ...
    card.addEventListener('drop', (e) => {
      ...
      if (draggedElement && dropTarget && draggedElement !== dropTarget) {
        ...
        console.log('Reordering:', draggedCategory, 'relative to', targetCategory);
        ...
        const modalInstance = modalElement ? modalElement.modalInstance || categoryManagerModalInstance : categoryManagerModalInstance;

        // FIXED: Refresh the modal but preserve the open state of any expanded sections
        const openSubcategories = [];
        container.querySelectorAll('.subcategories-section[style*="block"]').forEach(section => {
        ...
        openSubcategories.forEach(categoryName => {
          const categoryCard = container.querySelector('[data-category="' + categoryName + '"]');
          if (categoryCard) {
            const subcategoriesSection = categoryCard.querySelector('.subcategories-section');
            if (subcategoriesSection) {
              subcategoriesSection.style.display = 'block';
              const expandBtn = categoryCard.querySelector('.btn-subcategories .expand-icon');
              if (expandBtn) expandBtn.textContent = '▲';
              attachSubcategoryEventListeners(subcategoriesSection, container, modalInstance);
            }
          }
        });
 ...
 */
function reorderCategories(draggedCategoryName, targetCategoryName, insertAfter) {
  const categories = AppState.categories || {};
  ...
  const categoryEntries = Object.entries(categories).map(([name, data]) => ({
    name,
    data,
    order: (typeof data === 'object' && data.order !== undefined) ? data.order : 999
  })).sort((a, b) => a.order - b.order);

  // Find dragged and target positions
  const draggedIndex = categoryEntries.findIndex(cat => cat.name === draggedCategoryName);
  const targetIndex = categoryEntries.findIndex(cat => cat.name === targetCategoryName);
...
function handleManageSubcategories(categoryName) {
  ...
  let html = '<div class="subcategory-header">' +
  ...
  attachSubcategoryModalEventListeners(modalContent, subcategoryModal, categoryName);
}
...
function refreshCategoryDropdowns() {
  ...
  const categoryFilters = document.querySelectorAll('#categoryFilter, .category-select-btn');
  categoryFilters.forEach(filter => {
    // Trigger a refresh of the advanced filters if they exist
    if (window.refreshAdvancedFilters && typeof window.refreshAdvancedFilters === 'function') {
...
export function addSubcategory(parentName, subName, subColor) {
  ...
  if (!AppState.categories[parentName]) {
    import('./uiManager.js').then(module => {
      module.showToast('Parent category "' + parentName + '" not found. Please create it first.', 'error');
    });
    return false;
  }
  ...
  AppState.categories[parentName].subcategories[subName] = subColor || '#e74c3c';
  localStorage.setItem('categories', JSON.stringify(AppState.categories));
  refreshCategoryDropdowns();
...
export function updateSubcategory(parentName, oldSubName, newSubName, newSubColor) {
  ...
  if (newSubName !== oldSubName && AppState.categories[parentName].subcategories[newSubName]) {
    import('./uiManager.js').then(module => {
  ...
  if (newSubName !== oldSubName) {
    ...
    if (AppState.transactions) {
      AppState.transactions.forEach(transaction => {
        if (transaction.category === parentName && transaction.subcategory === oldSubName) {
  ...
  import('./uiManager.js').then(module => {
    module.showToast('Subcategory "' + newSubName + '" updated successfully', 'success');
  });
...
export function deleteSubcategory(parentName, subName) {
  ...
  if (AppState.transactions) {
    AppState.transactions.forEach(transaction => {
      if (transaction.category === parentName && transaction.subcategory === subName) {
        transaction.subcategory = '';
      }
    });
    localStorage.setItem('transactions', JSON.stringify(AppState.transactions));
 ...
 */
function attachSubcategoryEventListeners(subcategoriesSection, container, modal) {
  ...
  subcategoriesSection.querySelectorAll('.btn-edit-subcategory').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const subcategoryCard = e.target.closest('.subcategory-card');
      const editForm = subcategoryCard.querySelector('.subcategory-edit-form');
  ...
  subcategoriesSection.querySelectorAll('.btn-save-subcategory').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const parentName = e.target.getAttribute('data-parent');
      const oldSubName = e.target.getAttribute('data-subcategory');
      const subcategoryCard = e.target.closest('.subcategory-card');
      const nameInput = subcategoryCard.querySelector('.edit-subname-input');
      const colorInput = subcategoryCard.querySelector('.edit-subcolor-input');

      if (nameInput && colorInput) {
        const newSubName = nameInput.value.trim();
        const newSubColor = colorInput.value;

        if (newSubName && updateSubcategory(parentName, oldSubName, newSubName, newSubColor)) {
          // Refresh the modal content
          container.innerHTML = buildCategoryManagerHTML();
          attachCategoryManagerEventListeners(container, modal);
        }
  ...
  subcategoriesSection.querySelectorAll('.btn-cancel-subcategory').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const subcategoryCard = e.target.closest('.subcategory-card');
      const editForm = subcategoryCard.querySelector('.subcategory-edit-form');
      if (editForm) {
  ...
  subcategoriesSection.querySelectorAll('.btn-delete-subcategory').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const parentName = e.target.getAttribute('data-parent');
