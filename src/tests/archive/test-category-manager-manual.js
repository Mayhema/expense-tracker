...
function testCategoryManagerManually() {
  ...
  try {
    ...
    if (missingDragDrop.length === 0) {
      ...
      console.log(
        "❌ Missing drag and drop features:",
        missingDragDrop.join(", ")
      );
      allTestsPassed = false;
    }
    ...
    buttonTypes.forEach((btnType) => {
      if (
        !categoryManagerCode.includes(btnType) ||
        !categoryManagerCode.includes(btnType + ".addEventListener")
      ) {
        ...
        const pattern2 = new RegExp(
          "querySelector.*" + btnType + ".*addEventListener",
          "g"
        );
        if (
          !pattern1.test(categoryManagerCode) &&
          !pattern2.test(categoryManagerCode)
        ) {
    ...
    if (missingButtonHandlers.length === 0) {
      ...
      console.log(
        "❌ Missing button handlers for:",
        missingButtonHandlers.join(", ")
      );
      allTestsPassed = false;
    }
    ...
    const duplicateChecks = [
      {
        pattern: /addEventListener\('drop'/g,
        name: "drop event listeners",
        maxCount: 1,
      },
      {
        pattern: /function setupCategoryDragAndDrop/g,
        name: "setupCategoryDragAndDrop function",
        maxCount: 1,
      },
      {
        pattern: /function reorderCategories/g,
        name: "reorderCategories function",
        maxCount: 1,
      },
    ];
    ...
    duplicateChecks.forEach((check) => {
      ...
      if (matches && matches.length > check.maxCount) {
        duplicatesFound.push(
          `${check.name} (found ${matches.length}, expected ${check.maxCount})`
        );
      }
    });
