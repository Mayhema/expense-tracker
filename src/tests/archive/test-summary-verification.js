    ...
    if (id === "transactionSummary") {
      return {
        ...
        set innerHTML(value) {
          ...
          console.log(
            `📋 Transaction summary updated: ${value.slice(0, 100)}...`
          );
        },
      };
    }
...
AppState.transactions.forEach((tx, index) => {
  console.log(
    `  ${index + 1}. ${tx.description} - ${CURRENCIES[tx.currency].icon} ${
      tx.currency
    } (${tx.income > 0 ? "+" + tx.income : "-" + tx.expenses})`
  );
});
...
const transactionToChange = AppState.transactions.find(
  (tx) => tx.id === "tx-001"
);
if (transactionToChange) {
  console.log(
    `📝 Changing transaction "${transactionToChange.description}" from ${transactionToChange.currency} to GBP`
  );
  ...
  if (currencyGroups["GBP"] && currencyGroups["GBP"].expenses === 120.5) {
...
allCurrencies.forEach((currency) => {
  const filteredTx = AppState.transactions.filter(
    (tx) => tx.currency === currency
  );
  console.log(
    `📈 ${currency} chart data would include ${filteredTx.length} transactions`
  );
