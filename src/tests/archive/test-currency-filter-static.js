...
function testCurrencyFilter() {
  ...
  console.log('\n=== Test 1: Filter by USD ===');
  const usdTransactions = AppState.transactions.filter(tx => tx.currency === 'USD');
  ...
  console.log('USD transactions:', usdTransactions.map(tx => `${tx.description} (${tx.currency})`));

  // Test 2: Filter by EUR
  console.log('\n=== Test 2: Filter by EUR ===');
  const eurTransactions = AppState.transactions.filter(tx => tx.currency === 'EUR');
  console.log(`EUR filtered transactions: ${eurTransactions.length}`);
  console.log('EUR transactions:', eurTransactions.map(tx => `${tx.description} (${tx.currency})`));

  // Test 3: Filter by GBP
  console.log('\n=== Test 3: Filter by GBP ===');
  const gbpTransactions = AppState.transactions.filter(tx => tx.currency === 'GBP');
  console.log(`GBP filtered transactions: ${gbpTransactions.length}`);
  console.log('GBP transactions:', gbpTransactions.map(tx => `${tx.description} (${tx.currency})`));

  // Test 4: All currencies
  console.log('\n=== Test 4: All currencies ===');
  const allCurrencies = [...new Set(AppState.transactions.map(tx => tx.currency))];
  ...
  allCurrencies.forEach(currency => {
    const currencyTransactions = AppState.transactions.filter(tx => tx.currency === currency);
    const totalIncome = currencyTransactions.reduce((sum, tx) => sum + (tx.income || 0), 0);
    const totalExpenses = currencyTransactions.reduce((sum, tx) => sum + (tx.expenses || 0), 0);
    const netBalance = totalIncome - totalExpenses;

    console.log(`${currency}: Income: ${totalIncome}, Expenses: ${totalExpenses}, Net: ${netBalance}`);
  });
