...
global.AppState = {
  ...
  transactions: [
    ...
    { category: "Transport", amount: 15.3 },
  ],
};
  ...
  const tests = [
    {
      ...
      test: () => {
        try {
          const categoryManagerCode = fs.readFileSync(
            "src/ui/categoryManager.js",
            "utf8"
          );

          return (
            categoryManagerCode.includes("setupCategoryDragAndDrop") &&
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          const hasAddCategoryBtn = categoryManagerCode.includes(
            "addCategoryBtn.addEventListener"
          );
          const hasResetBtn = categoryManagerCode.includes(
            "resetCategoriesBtn.addEventListener"
          );
          const hasCloseBtn =
            categoryManagerCode.includes("closeCategoryManagerBtn") ||
            categoryManagerCode.includes("closeBtn.addEventListener");
          const hasEditBtns =
            categoryManagerCode.includes("btn-edit") &&
            categoryManagerCode.includes("addEventListener");
          const hasDeleteBtns =
            categoryManagerCode.includes("btn-delete") &&
            categoryManagerCode.includes("addEventListener");
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          const hasModernGradients =
            cssCode.includes("linear-gradient") && cssCode.includes("#6366f1");
          const hasAnimations =
            cssCode.includes("@keyframes") && cssCode.includes("animation:");
          const hasTransitions =
            cssCode.includes("transition:") && cssCode.includes("cubic-bezier");
          const hasModernBorderRadius =
            cssCode.includes("border-radius: 24px") ||
            cssCode.includes("border-radius: 20px");
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          const hasModernBtnStyles =
            cssCode.includes(".btn") && cssCode.includes("cubic-bezier");
          const hasHoverEffects =
            cssCode.includes(".btn:hover") && cssCode.includes("translateY");
          const hasGradientBtns =
            cssCode.includes(".btn-primary") &&
            cssCode.includes("linear-gradient");
          const hasAnimatedBtns =
            cssCode.includes("gradientMove") || cssCode.includes("animation:");
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          const dropMatches = categoryManagerCode.match(
            /addEventListener\('drop'/g
          );
          const dragoverMatches = categoryManagerCode.match(
            /addEventListener\('dragover'/g
          );

          // Should have exactly one of each per card setup
          return (
            dropMatches &&
            dropMatches.length <= 2 &&
            dragoverMatches &&
            dragoverMatches.length <= 2
          );
        } catch (error) {
          console.error("Error reading categoryManager.js:", error.message);
          throw error; // Re-throw to handle properly
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          return (
            categoryManagerCode.includes("console.log") &&
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          return (
            categoryManagerCode.includes("categoryManagerModalInstance") &&
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          return (
            categoryManagerCode.includes("function reorderCategories") &&
    ...
    },
    {
      ...
      test: () => {
        try {
          ...
          return (
            categoryManagerCode.includes("showToast") &&
...
// Run the tests
describe("test-category-manager-comprehensive", () => {
  test("minimal category manager comprehensive test passes", () => {
