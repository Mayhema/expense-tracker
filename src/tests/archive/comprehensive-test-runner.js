...
async function main() {
  ...
  const rootTestFiles = findTestFiles(projectRoot).filter(file =>
  ...
  const testCategories = {
    'Category Manager': passed.concat(failed).filter(r => r.file.includes('category')),
    'Filters': passed.concat(failed).filter(r => r.file.includes('filter')),
    'Transactions': passed.concat(failed).filter(r => r.file.includes('transaction')),
    'Currency': passed.concat(failed).filter(r => r.file.includes('currency')),
    'Integration': passed.concat(failed).filter(r => r.file.includes('integration')),
    'Regression': passed.concat(failed).filter(r => r.file.includes('regression')),
    'UI/UX': passed.concat(failed).filter(r => r.file.includes('ui') || r.file.includes('layout')),
    'Other': passed.concat(failed).filter(r =>
  ...
  Object.entries(testCategories).forEach(([category, tests]) => {
    if (tests.length > 0) {
      ...
      const status = categoryFailed === 0 ? colors.green + '✅' : colors.yellow + '⚠️';
      console.log(`   ${status} ${category} (${categoryPassed}/${tests.length})${colors.reset}`);
    }
  });
  ...
  if (failed.length > 0) {
    ...
    console.log('');
    console.log(`${colors.yellow}   Please review the failed tests above for details.${colors.reset}`);
  } else {
    console.log('');
