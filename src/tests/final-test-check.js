...
try {
  ...
  const passedRegex = /(\d+) passed/;
  const failedRegex = /(\d+) failed/;
  const totalRegex = /(\d+) total/;
  const passedMatch = passedRegex.exec(result);
  const failedMatch = failedRegex.exec(result);
  const totalMatch = totalRegex.exec(result);
  ...
  const failedRegex = /(\d+) failed/;
  const totalRegex = /(\d+) total/;
  const passedMatch = passedRegex.exec(output);
  const failedMatch = failedRegex.exec(output);
