 ...
 */
export function validateRowIndices(
  data,
  headerInputId = "headerRowInput",
  dataInputId = "dataRowInput"
) {
  ...
  if (!isHeaderValid) {
    console.warn(
      `Header row ${headerRow + 1} is out of range (1-${data.length})`
    );
    showValidationError(
      headerInput,
      `Row must be between 1 and ${data.length}`
    );
    return false;
  }
...
function validateTransactionAmounts(transaction) {
  ...
  const hasExpenses =
    transaction.expenses && parseFloat(transaction.expenses) > 0;
  ...
  if (
    transaction?.income &&
    (isNaN(parseFloat(transaction.income)) ||
      parseFloat(transaction.income) < 0)
  ) {
  ...
  if (
    ...
    (isNaN(parseFloat(transaction.expenses)) ||
      parseFloat(transaction.expenses) < 0)
  ) {
...
export function generateTransactionId(index = 0) {
  return `tx_${Date.now()}_${Math.random()
    .toString(36)
...
export function validateFileData(data) {
  ...
  if (inconsistentRows > data.length * 0.1) {
    ...
    errors.push(
      `Too many rows with inconsistent column counts (${inconsistentRows} out of ${
        data.length - 1
      })`
    );
  }
  ...
  return {
    ...
    warnings:
      inconsistentRows > 0
        ? [`${inconsistentRows} rows have inconsistent column counts`]
        : [],
  };
}
...
export function validateHeaderMapping(mapping) {
  ...
  if (!mapping.includes("Description")) {
    warnings.push(
      "No Description column mapped - transactions may be harder to identify"
    );
  }
...
export function validateDateMapping(mapping, newIndex, newValue) {
  ...
  const dateCount = mapping.filter(
    (field, index) => field === "Date" && index !== newIndex
  ).length;
